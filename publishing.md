...
- [ ] The release body is auto-generated by the workflow: a readable summary
      (auto-updates, NEWS tab, portable chip, localization…) is **prepended**
      to the full commit changelog — no manual writing needed. (If the
      release predates the template, PATCH the body once via
      `scripts/gh-publish.cjs`.)
- [ ] If Azure Trusted Signing secrets are set: exes are signed (SmartScreen
      ...
      (Signing only affects tags built AFTER the secrets are added; to get a
      signed build of an already-released tag, delete + recreate the tag or
